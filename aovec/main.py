...
def make_argparser() -> argparse.ArgumentParser:
    ...
    parser_clone = subparsers.add_parser(
        "clone",
        aliases=["c"],
        help="clone aozorabunko/aozorabunko (>20GB)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    ...
    parser_parse = subparsers.add_parser(
        "parse",
        aliases=["p"],
        help="parse html files and write to results",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    ...
    parser_mkvec = subparsers.add_parser(
        "mkvec",
        aliases=["m"],
        help="make word2vec and write to *.model",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser_mkvec.add_argument(
        ...
        default="novels",
        metavar="DIR",
        help="directory name of saved parsing results",
    )
    parser_mkvec.add_argument(
        "-o",
        "--model",
        default="aozora_model",
        metavar="NAME",
        help="name of word2vec model",
    )
    parser_mkvec.add_argument(
        "-e",
        "--epochs",
        ...
        type=check_positive,
        metavar="INT",
        help="number of word2vec epochs",
    )
    parser_mkvec.add_argument(
        "-v",
        "--vector_size",
    ...
    parser_mkvec.add_argument(
        ...
        type=check_positive,
        metavar="INT",
        help="ignore words total frequency lower than this",
    )
    parser_mkvec.add_argument(
        "-w",
        "--window",
        ...
        type=check_positive,
        metavar="INT",
        help="window size of words before and for learning",
    )
    parser_mkvec.add_argument(
        "-p",
        "--workers",
    ...
    parser_mkvec.add_argument(
        "-b", "--binary", action="store_true", help="save model files as one binary"
    )
    parser_mkvec.add_argument(
        "--both",
        action="store_true",
